...
def bm25(qry, docs):
    ...
    # Create a nested list for the list of documents where each document is a list having title and text [[title1,text1],[title2,text2]....]
    # Even line is the title,odd is text
    # Each article has title and text of one doc,doc has list of all docs
    '''
    for line in f:
        if count%2==0:
            article.append(line.strip().split('\n'))
            words=word_tokenize(''.join(article[0]))
            ...
            article.append(line.strip().split('\n'))
            doc.append(article)
            words=word_tokenize(''.join(article[1]))
            all_words.append(words)
            temp=temp+len(words)
